  ...
  componentWillReceiveProps(nextProps, nextContext) {
    if (nextProps.type === 1) {
      axios({
        ...
        method: 'get',
        headers: {
          'X-Client-Info': '{"a":"3000","ch":"1002","v":"5.2.1","e":"16940691196886675116457985"}',
...
export default class App extends Component {
  ...
  render() {
    return (
        <div>
          <ul>
            <li style={{borderBottom: this.state.type === 1 ? '2px solid red' : ''}} onClick={() => {
              this.setState({
                type: Number(1)
              })
            }}>正在热映
            </li>
            <li style={{borderBottom: this.state.type === 2 ? '2px solid red' : ''}} onClick={() => {
              this.setState({
                type: Number(2)
              })
            }}>即将上映
